"""
AI Sommelier의 이미지 분석/검색/추천하는 RAG 파이프라인
...
CHAT_MODEL_NAME = os.getenv("CHAT_MODEL_NAME",'gpt-5.6-luna').strip() or 'gpt-5.6-luna'
...
PINECONE_INDEX_NAME = os.getenv('SOMMELIER_PINECONE_INDEX',"winemag-review-data").strip() or 'winemag-review-data'
...
def describe_dish_flavor(payload: dict[str, Any]) -> Runnable:
    '''이미지 URL을 받아서 영어 한 문장의 풍미 query를 생성한 Runnable 반환'''
...
def search_wines(dish_flavor: str) -> dict[str, str]:
    ...
    # 최종 답변 생성 LLM(==Generation)에 전달할 query  반환
    return {
